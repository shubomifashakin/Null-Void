...
export default function ProfilePage() {
  ...
  const { mutate: deleteMyAccount, isPending: isDeleting } = useMutation({
    ...
    onError: (error) => {
      ...
      if (error.cause === 403) return toast.error("Forbidden");
      if (error.cause === 429)
  ...
  const { mutate: updateMyAccountInfo, isPending: isUpdating } = useMutation({
    ...
    onError: (error) => {
      if (error.cause === 400) return toast.error(error.message);
      if (error.cause === 401) {
        toast.error("Unauthorized");
        return router.push("/");
      }
  ...
  return (
    <AccountInfo
      ...
      handleDeleteAccount={() => deleteMyAccount()}
      handleUpdateAccountInfo={({ name }) => updateMyAccountInfo({ name })}
    />
  );
}
